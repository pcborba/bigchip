<?php
	...
	$sql_read = "SELECT Accounts.title as 'GLAccountName', CONCAT('$ ',ROUND(SUM(EntryItems.value),2)) as 'TotalValue' FROM entries Entries INNER JOIN entry_items EntryItems ON Entries.id = EntryItems.entry_id INNER JOIN gl_accounts Accounts ON EntryItems.gl_id = Accounts.id WHERE Entries.date >= '".$start ."' AND Entries.date <= '".$end."' GROUP BY Accounts.title ORDER BY Accounts.title";
