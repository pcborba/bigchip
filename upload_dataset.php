...
<?php
/* These file contain the script to upload the excel file
and also insert data from that file on the database   */
...
// if everything is ok, try to upload file
    ...
    if (!move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        ...
        //We just call the function import data if a new excel file is approved and uploaded
...
/*
This function read each sheet contained in the spreadsheet file
and insert the data on database for do that we are using an
external PHP library available on Git in the follow link
https://github.com/PHPOffice/PhpSpreadsheet the documentation
to use is available in the link https://phpspreadsheet.readthedocs.io/en/develop/
*/
function importData(){
    ...
    $tablesCreated = 0;
    //instantiating the object that represent our spreadsheet
    $reader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader("Xlsx");

    //loading the file and retrieving the sheets
    ...
    //setting the format to parse the date and datetime
    $formatDate = 'Y-m-d';
    $formatDateTime = 'Y-m-d H:i';

    //Iterating inside the sheet and retrieving the content
    foreach($sheetLocations->getRowIterator(2) as $row){
        ...
        //creating the query for each row 
        $sql_insert = "INSERT INTO locations (id, location) VALUES (:ID, :LOCATION)";

        //inserting the content of each row into database
        try{
    ...
    //couting how many sheets were successfully uploaded
    $tablesCreated += 1;
